...
void ResponsiveLayout::setGeometry(const QRect &r /* our layout should always fit inside r */ ) {
    ...
    int count_500kSR = 0;               //global counter-variable for "kSearchResult" when "r.width() < 500"
    int x_500kSR = r.x() + 6;           //starting x-coordinate for "kSearchResult" when "r.width() < 500" is 6
    int y_500kSR = r.y() + 60;          //starting y-coordinate for "kSearchResult" when "r.width() < 500" is 60

    int count_800kSR = 0;               //global counter-variable for "kSearchResult" when "r.width() >= 500 && r.width() < 800"
    int x_800kSR = r.x() + 109;         //starting x-coordinate for "kSearchResult" when "r.width() >= 500 && r.width() < 800" is 109
    int y_800kSR = r.y() + 75;          //starting y-coordinate for "kSearchResult" when "r.width() >= 500 && r.width() < 800" is 75
    ...
    int count_500kRT = 0;               //global counter-variable for "kResultText" when "r.width() < 500"
    int x_500kRT = r.x() + 6;           //starting x-coordinate for "kResultText" when "r.width() < 500" is 6
    int y_500kRT = r.y() + 146;         //starting y-coordinate for "kResultText" when "r.width() < 500" is 146

    int count_800kRT = 0;               //global counter-variable for "kResultText" when "r.width() >= 500 && r.width() < 800"
    int x_800kRT = r.x() + 109;         //starting x-coordinate for "kResultText" when "r.width() >= 500 && r.width() < 800" is 109
    int y_800kRT = r.y() + 186;         //starting y-coordinate for "kResultText" when "r.width() >= 500 && r.width() < 800" is 186
    ...
    for (int i = 0; i < list_.size(); i++)
    {
        ...
        try {
            ...
            else if (label->text() == kSignIn)
            {
                ...
                else if (r.width() >= 800 && r.width() < 1024)
                {
            ...
            else if (label->text() == kSearchText)
            {
                if (r.width() < 800)
                {
                    label->setGeometry(r.x() + 2, r.y() + 32, r.width() - 115, 20);     //positions at (2, 32) dimensions: (r.width() - 115)x20
                }
                else if (r.width() >= 800 && r.width() < 1024)
                {
            ...
            else if (label->text() == kWishlist)
            {
                if (r.width() < 800)
                {
                    label->setGeometry(r.width() - 103 + r.x(), r.y() + 55, 101, 15);     //positons at (r.width() - 70, 55) dimensions: 68x15
                }
                else if (r.width() >= 800 && r.width() < 1024)
                {
            ...
            else if (label->text() == kSearchOptions)
            {
                if (r.width() < 500)
                {
                    label->setGeometry(r.width() - 103 + r.x(), r.y() + 75, 100, r.height() - 100);   //positions at (r.width() - 70, 75), Right-Hand-side, dimensions: 68x(r.height() - 100)
                }
                else if (r.width() >= 500)
                {
                    label->setGeometry(r.x() + 2, r.y() + 75, 100, r.height() - 100);                //positions at (2, 75) , Left-Hand-side, dimensions: 68x(r.height()-100)
                }
            }
            ...
            else if (label->text() == kShoppingBasket)
            {
                if (r.width() < 800)
                {
                    label->setGeometry(r.width() - 62 + r.x(), r.y() + 32, 60, 20);                 //positions at (r.width() - 62, 32) dimensions: 60x20
                }
                else if (r.width() >= 800 && r.width() < 1024)
                {
                    label->setGeometry(r.width() - 62 + r.x(), r.y() + 42, 60, 30);                 //positions at (r.width() - 62. 42) dimensions: 60x30
                ...
                {
                    label->setGeometry(r.width() - 138 + r.x(), r.y() + 42, 73, 30);                //positions at (r.width() - 138, 42) dimensions: 73x30
                }
            }
            ...
            else if (label->text() == kSearchBackward)
            {
                if (r.width() < 500)
                {
                    label->setGeometry(r.width() - 103 + r.x(), r.height() + r.y() - 20, 40, 15);    //positions at (r.width() - 70, r.height() - 20) dimensions: 30x15
                }
                else if (r.width() >= 500)
                {
                    label->setGeometry(r.width() - 123 + r.x(), r.height() - r.y() - 20, 40, 15);   //positions at (r.width() - 123, r.height() - 30) dimensions: 40x15
                }
            }
            ...
            else if (label->text() == kSearchForward)
            {
                if (r.width() < 500)
                {
                    label->setGeometry(r.width() - 43 + r.x(), r.height() + r.y() - 20, 40, 15);    //positions at (r.width() - 35, r.height() - 20) dimensions: 30x15
                }
                else if (r.width() >= 500)
                {
                    label->setGeometry(r.width() - 43 + r.x(), r.height() - r.y() - 20, 40, 15);    //positions at (r.width() - 43, r.height() - 20) dimensions: 40x15
                }
            }
            ...
            {
                count_500kSR++;     //counter increments whenever this widget is found in the "ResponsiveLabel"

                if (count_500kSR == 1)
                {
                    label->setGeometry(x_500kSR, y_500kSR, 90, 115);     //manually positions the first widget at (6, 60) with dimensions 90x115
                }

                else if (count_500kSR > 1)
                {
                    x_500kSR = x_500kSR + 103;      //increases width-gap by 103 units to the right

                    if (x_500kSR + 160 > r.width() - 40)    //validates that the new x-coordinate exceeds 40 units to the right
                    {
                        y_500kSR = y_500kSR + 159;  //increases width-gap by 159 units to the right
                        x_500kSR = r.x() + 6;       //pushes the widget 6 units to the right

                        if (y_500kSR + 70 < r.height() - 74)                    //validates that new y-coordinate exceeds roughly 74 units above the window
                        {
                            label->setGeometry(x_500kSR, y_500kSR, 90, 115);    //positions the widget in new (x, y) coordinates with the same dimension
                        }
                        else
                        {
                            label->setGeometry(x_500kSR, y_500kSR, 0, 0);       //hides the widget
                        }
                    }
                    else
                    {
                        if (y_500kSR + 70 > r.height() - 74)                    //validates that new y-coordinate exceeds roughly 74 units below the window
                        {
                            label->setGeometry(x_500kSR, y_500kSR, 0, 0);       //hides the widget
                        }
                        else
            ...
            {
                count_500kRT++; //counter increments whenever this widget is found in the "ResponsiveLabel"

                if (count_500kRT == 1)
                {
                    label->setGeometry(x_500kSR, y_500kSR + 119, 90, 30);   //manually positions the first widget at (6, 179) with dimensions 90x30
                }

                else if (count_500kRT > 1)
                {
                    x_500kRT = x_500kRT + 103;  //increases width-gap by 103 units to the right

                    if (x_500kRT + 160 > r.width() - 40)    //validates that the new x-coordinate exceeds 40 units to the right
                    {
                        y_500kRT = y_500kRT + 159;      //increases height-gap by 159 units below
                        x_500kRT = r.x() + 6;           //pushes the widget 6 units to the right

                        if (y_500kRT + 60 < r.height() - 10)                        //validates that new y-coordinate exceeds roughly 10 units above the window
                        {
                            label->setGeometry(x_500kRT, y_500kRT + 33, 90, 30);    //positions the widget in new (x, y) coordinates with the same dimension
                        }
                        else
                        {
                            label->setGeometry(x_500kRT, y_500kRT, 0, 0);       //hides the widget
                        }
                    }
                    else
                    {
                        if (y_500kRT + 60 > r.height() - 10)                    //validates that new y-coordinate exceeds roughly 10 units below the window
                        {
                            label->setGeometry(x_500kRT, y_500kRT, 0, 0);       //hides the widget
                        }
                        else
            ...
            //kSearchResult_MEDIUM
            else if (label->text() == kSearchResult && r.width() >= 500 && r.width() < 800)
            {
                count_800kSR++;     //counter increments whenever this widget is found in the "ResponsiveLabel"

                if (count_800kSR == 1)
                {
                    label->setGeometry(x_800kSR, y_800kSR, 90, 115);     //manually positions the first widget at (109, 75) with dimensions 90x135
                }
                ...
                else if (count_800kSR > 1)
                {
                    ...
                    if (x_800kSR + 160 > r.width() - 65)    //validates that the new x-coordinate exceeds 65 units to the right
                    {
                        y_800kSR = y_800kSR + 159;  //increases width-gap by 159 units to the right
                        x_800kSR = r.x() + 109;       //pushes the widget 109 units to the right
            ...
            //kResultText_MEDIUM
            else if (label->text() == kResultText && r.width() >= 500 && r.width() < 800)
            {
                count_800kRT++; //counter increments whenever this widget is found in the "ResponsiveLabel"

                if (count_800kRT == 1)
                {
                    label->setGeometry(x_800kRT, y_800kRT + 7, 90, 30);   //manually positions the first widget at (109, 193) with dimensions 90x30
                }
                ...
                else if (count_800kRT > 1)
                {
                    ...
                    if (x_800kRT + 160 > r.width() - 65)    //validates that the new x-coordinate exceeds 65 units to the right
                    {
                        y_800kRT = y_800kRT + 159;      //increases height-gap by 159 units below
                        x_800kRT = r.x() + 109;           //pushes the widget 6 units to the right

                        if (y_800kRT < r.height() - 10)                        //validates that new y-coordinate exceeds roughly 10 units above the window
                        {
                            label->setGeometry(x_800kRT, y_800kRT + 7, 90, 30);    //positions the widget in new (x, y) coordinates with the same dimension
                        }
                        else
                        {
                    ...
                    {
                        if (y_800kRT > r.height() - 10)                    //validates that new y-coordinate exceeds roughly 10 units below the window
                        {
                            label->setGeometry(x_800kRT, y_800kRT, 0, 0);       //hides the widget
                        }
                        else
            ...
            else if (label->text() == kSearchResult && r.width() >= 800)
            {
                ...
                if (count_1280kSR == 1)
                {
                    label->setGeometry(x_1280kSR + 30, y_1280kSR, 110 , 135);      //manually positions the first widget at (80, 76) with dimensions 110x135
                }
                ...
                else if (count_1280kSR > 1)
                {
                    ...
                    if (x_1280kSR + 163 > r.width() - 107)   //validates that the new x-coordinate exceeds 107 units to the right
                    {
                        ...
                        {
                            label->setGeometry(x_1280kSR + 30, y_1280kSR, 110, 135);     //postions the widget in new (x, y) coordinates with the same dimension
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            label->setGeometry(x_1280kSR + 30, y_1280kSR, 110, 135);     //positions the widget in new (x, y) coordinates with the same dimension
                        }
                    }
                }
            }
            ...
            else if (label->text() == kResultText && r.width() >= 800)
            {
                ...
                if (count_1280kRT == 1)
                {
                    label->setGeometry(x_1280kRT + 29, r.y() + 214, 110, 35);       //manually positions the first widget at (109, 214) with dimensions 110x135
                }
                else if (count_1280kRT > 1)
                {
                    x_1280kRT = x_1280kRT + 124;    //increases width-gap by 124 units to the right

                    if (x_1280kRT + 163 > r.width() - 107)   //validates that the new x-coordinate exceeds 107 units to the right
                    {
                        ...
                        {
                            label->setGeometry(x_1280kRT + 30, y_1280kRT + 25, 110,  35);    //positions the widget in new (x, y) coordinates with the same dimension
                        }
                        else
                        {
